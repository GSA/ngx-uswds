...
@Component({
  ...
  template: `
    <usa-checkbox
      *ngIf="props.selectAllLable"
      [checked]="allChecked"
      [indeterminate]="checkIndeterminate()"
      (change)="checkAll($event)"
      >{{ props.selectAllLable }}</usa-checkbox
    >

    <ul class="usa-list usa-list--unstyled">
      <li class="margin-left-3">
        <usa-checkbox
          *ngFor="let item of props.options"
          [tile]="props.tile"
          (change)="onChange(item.key, item.checked)"
          [(ngModel)]="item.checked"
          >{{ item.value }}</usa-checkbox
        >
      </li>
    </ul>
  `,
...
export class USWDSFormlyMultiCheckboxComponent extends AbstractUswdsFormly {
  ...
  checkIndeterminate() {
    ...
    if (Array.isArray(this.field.props.options)) {
      this.allChecked = value === this.field.props.options.length ? true : false;
    }
  }
