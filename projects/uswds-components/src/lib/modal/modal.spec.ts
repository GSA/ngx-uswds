...
describe('UsaModal', () => {
  ...
  let openModal = () => {
    const openButton: HTMLButtonElement =
      component._el.nativeElement.querySelector('#modal-test-open');
    openButton.click();
  };
  ...
  it('Should open and close modal', (done: DoneFn) => {
    ...
    component.modalRef.shown.toPromise().then(() => {
      const overlayModel: HTMLElement =
        document.body.querySelector('.usa-modal-overlay');
      expect(overlayModel).toBeDefined();
      const closeButton: HTMLButtonElement =
        overlayModel.querySelector('#modal-test-close');
      closeButton.click();
    });
...
@Component({
  template: `
    ...
    <button class="usa-button" id="modal-test-open" (click)="open(content)">
...
class UsaModalTestComponent {
  ...
  constructor(private modalService: UsaModalService, public _el: ElementRef) {}
