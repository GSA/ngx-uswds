 ...
 */
export const buildFileInput = (fileInputEl) => {
 ...
 * @param {HTMLElement} dropTarget - target area div that encases the input
 */
export const preventInvalidFiles = (e, fileInputEl, instructions, dropTarget) => {
 ...
 * @param {HTMLElement} dropTarget - target area div that encases the input
 */
export const handleChange = (e, fileInputEl, instructions, dropTarget) => {
  ...
  for (let i = 0; i < fileNames.length; i += 1) {
    ...
    reader.onloadend = function createFilePreview() {
      const imageId = makeSafeForID(fileName);
      const previewImage = document.getElementById(imageId) as any;
