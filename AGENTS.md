...
- Unit tests run on **Vitest + jsdom** through Angular's `@angular/build:unit-test` builder (Karma/Jasmine were removed). Specs use Vitest globals (`vi`, `expect`, `describe`, `it`); prefer importing a component's `NgModule` over bare `declarations: [...]`, because the builder compiles AOT with `errorOnUnknownElements`/`errorOnUnknownProperties` enabled and cannot be relaxed.
